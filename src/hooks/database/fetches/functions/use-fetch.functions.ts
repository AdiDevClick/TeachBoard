...
export function cacheFetchResult(
  ...
  const { dataReshapeFn, reshapeOptions, cachedFetchKey, contentId } =
    fetchParams;

  // build the final URL (with search params) so that our cache key matches the
  // one generated by useFetch when the mutation runs.
  const urlWithParams = createSearchParamsEndpoint(fetchParams);
  // !! IMPORTANT !! Always override with cachedFetchKey if provided
  // as it can differ from the default [contentId, url]
  const cachedKey = cachedFetchKey ?? [contentId, urlWithParams];
